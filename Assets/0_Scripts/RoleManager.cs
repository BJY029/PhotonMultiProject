...
public class RoleManager : MonoBehaviourPun
{
    ...
    public GameObject RunnerIconPrefab;
	...
	private void Start()
	{
        ...
        switch (myRole)
        {
            case PlayerRoles.Seeker:
                ...
                MiniMapFollow.instance.player = playerObj.transform;
	...
	void SpawnRunner()
	{
        ...
        //해당 역할에 맞는 아이콘을 생성된 플레이어의 자식에 배치시켜주고,
        //적절한 위치 설정을 해준다.
        icon = Instantiate(RunnerIconPrefab, playerObj.transform);
	...
	void SpawnSeeker()
	{
		...
		//해당 역할에 맞는 아이콘을 생성된 플레이어의 자식에 배치시켜주고,
		//적절한 위치 설정을 해준다.
		icon = Instantiate(SeekerIconPrefab, playerObj.transform);
		icon.transform.rotation = Quaternion.Euler(90f, 0f, 0f);
		icon.transform.localPosition = new Vector3(0f, 10f, 0f);
